...
- [ ] 1. Database migration — Remove Prisma, install Mongoose
  - [ ] 1.1 Install Mongoose dependencies and remove Prisma
  ...
  - [ ] 1.2 Delete Prisma files and module
  ...
  - [ ] 1.3 Create DatabaseModule with Mongoose connection
  ...
  - [ ] 1.5 Update AppModule to use DatabaseModule
    - Replace `PrismaModule` import with `DatabaseModule` in `app.module.ts`
    - _Requirements: 4.1_

- [ ] 2. Checkpoint — Verify database module compiles
  - Run `pnpm --filter @certiq/api run build`
  - Ensure no Prisma references remain in API source

- [ ] 3. Auth module — Install better-auth with MongoDB adapter
  - [ ] 3.1 Install better-auth dependencies
    - Add `better-auth` to `apps/api` dependencies
    - _Requirements: 7.1_

  - [ ] 3.2 Create better-auth instance
  ...
  - [ ] 3.4 Rewrite AuthGuard with session validation
  ...
  - [ ] 3.7 Update environment variable files
    - Update `apps/api/.env.example` with all new variables
    - Update `apps/api/.env` with placeholder values
    - _Requirements: 7.6_

- [ ] 4. Checkpoint — Verify auth module compiles
  - Run `pnpm --filter @certiq/api run build`

- [ ] 5. Infrastructure modules — Redis and R2
  - [ ] 5.1 Install infrastructure dependencies
  ...
  - [ ] 5.2 Create CacheModule and CacheService
  ...
  - [ ] 5.3 Create StorageModule and StorageService
  ...
  - [ ] 5.4 Create file validation utility
...
- [ ] 6. Checkpoint — Verify infrastructure modules compile
  - Run `pnpm --filter @certiq/api run build`

- [ ] 7. Business modules — Update to Mongoose + Cache + R2
  - [ ] 7.1 Rewrite UsersModule with Mongoose
  ...
  - [ ] 7.2 Rewrite ResumesModule with Mongoose
  ...
  - [ ] 7.3 Rewrite CertificatesModule with Mongoose + R2 + Cache
  ...
  - [ ] 7.4 Create DTOs with validation
  ...
  - [ ] 7.5 Wire all modules in AppModule
...
- [ ] 8. Checkpoint — Verify full backend builds
  - Run `pnpm --filter @certiq/api run build`
  - Ensure all modules are properly wired

- [ ] 9. Frontend migration — Web App
  - [ ] 9.1 Install better-auth client in Web App
  ...
  - [ ] 9.2 Create auth-client.ts for Web App
  ...
  - [ ] 9.3 Delete InsForge client file
    - Delete `apps/web/lib/insforge.ts`
    - _Requirements: 9.1_

  - [ ] 9.4 Rewrite AuthProvider for Web App
...
- [ ] 10. Frontend migration — Dashboard App
  ...
  - [ ] 10.5 Update Dashboard components
  ...
  - [ ] 10.6 Remove InsForge env vars from Dashboard
    - Remove NEXT_PUBLIC_INSFORGE_URL and NEXT_PUBLIC_INSFORGE_ANON_KEY from .env.local and .env.example
    - _Requirements: 9.5_

- [ ] 11. Checkpoint — Verify frontend builds
  - Run `pnpm run build` across all apps
  - Verify no InsForge imports remain

- [ ] 12. InsForge final cleanup
  - [ ] 12.1 Remove InsForge from API
  ...
  - [ ] 12.2 Clean lockfile and verify
...
- [ ] 14. Optional property tests
  ...
  - [ ]* 14.3 Write property test for cache degradation (Property 11)
