...
export function LoginModal({ open, onClose }: LoginModalProps): JSX.Element | null {
  ...
  const { signInWithGoogle, signInWithLinkedIn, signInWithEmail, signUp, error } = useAuth();
  ...
  }, [email, password, name, isSignUp, signInWithEmail, signUp]);
      ...
      {open && (
        ...
        >
          ...
          >
            ...
            >
              {isSignUp
                ? "Sign up to start building your credentials"
            ...
            {error && (
              <motion.div
                className="w-full mb-4 px-4 py-3 rounded-lg bg-red-500/10 border border-red-500/20 text-red-400 text-sm font-sans text-center"
              ...
              >
                {isSignUp && (
                  <div className="flex flex-col gap-1.5">
                    <label htmlFor="login-name" className="text-text-muted font-sans text-xs font-medium">
                      Name
                    </label>
                    <input
                      id="login-name"
                      ...
                      required
                      className="w-full rounded-lg border border-border-card bg-surface-card-1 px-4 py-3 text-sm text-text-headline placeholder:text-text-muted/50 font-sans focus:outline-none focus:border-accent focus:ring-1 focus:ring-accent transition-colors"
                    />
                  </div>
                )}

                <div className="flex flex-col gap-1.5">
                  <label htmlFor="login-email" className="text-text-muted font-sans text-xs font-medium">
                    Email
                  </label>
                  <input
                    id="login-email"
                ...
                <div className="flex flex-col gap-1.5">
                  <label htmlFor="login-password" className="text-text-muted font-sans text-xs font-medium">
                    Password
                  </label>
                  <input
                    id="login-password"
                ...
                <button
                  type="submit"
                  disabled={submitting}
                  className="w-full rounded-full bg-accent text-white font-sans font-medium text-sm min-h-[50px] px-5 py-3 hover:bg-accent/90 transition-opacity disabled:opacity-50 disabled:cursor-not-allowed focus-visible:outline-2 focus-visible:outline-accent focus-visible:outline-offset-2"
                >
                  {submitting
                    ? "Please wait..."
                    : isSignUp
                ...
                <button
                  ...
                  className="text-text-muted font-sans text-xs text-center hover:text-text-headline transition-colors"
                >
                  ← Back to all options
                </button>
              </motion.form>
            ...
            >
              {isSignUp ? "Already have an account?" : "New to Certiq?"}{" "}
              <button
                type="button"
                onClick={() => {
                  setIsSignUp(!isSignUp);
                  setShowEmailForm(isSignUp ? false : showEmailForm);
                }}
                className="text-accent hover:underline font-medium focus-visible:outline-2 focus-visible:outline-accent focus-visible:outline-offset-2"
              >
